...
def find_board_config(image_path, board_file=None):
    assert board_file is None or path.isfile(
        board_file
    ), f"board file {board_file} not found"
...
def find_camera_images(
    ...
    camera_paths = image.find.find_cameras(
        image_path, cameras, camera_pattern, extensions=extensions
    )
    camera_names = list(camera_paths.keys())

    find_images = (
        image.find.find_images_matching
        if matching
        else image.find.find_images_unmatched
    )

    image_names, filenames = find_images(camera_paths, extensions=extensions)
    info(
        "Found camera directories {} with {} matching images".format(

...
class MovingCameras(object):
    def __init__(self, viewer, calib, board_colors):
        ...
        self.board_set = BoardSet(
            self.viewer, calib.pose_estimates.board, self.meshes.board, board_colors
        )
        self.camera_sets = [
        ...
        self.axis_set = AxisSet(
            self.viewer, self.meshes.axis, self.view_poses._index[:, 0]
        )
        self.show(False)

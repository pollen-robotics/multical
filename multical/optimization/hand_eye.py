...
class HandEyeCalibration:
    ...
    def initialise(calib, gripper_wrt_base):
        ...
        hand_eye_model = HandEye(
            table(poses=base_wrt_gripper, valid=valid),
            np.linalg.inv(base_wrt_world),
            gripper_wrt_camera,
        )

        calib = calib.copy(motion=hand_eye_model).enable(
            camera_poses=False, cameras=False
        )
    ...
    def bundle_adjust(self):
        return self.copy(calib=self.calib.bundle_adjust())

    def adjust_outliers(self, **kwargs):
        return self.copy(calib=self.calib.adjust_outliers(**kwargs))

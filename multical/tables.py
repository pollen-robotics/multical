...
def extract_pose(points, board, camera):
    ...
    return (
        valid_pose(rtvec.to_matrix(poses))._extend(num_points=len(detections.ids))
...
def initialise_poses(pose_table, camera_poses=None):
    ...
    if camera_poses is not None:
        ...
        camera = Table.create(
            poses=camera_poses, valid=np.ones(camera_poses.shape[0], dtype=np.bool)
        )

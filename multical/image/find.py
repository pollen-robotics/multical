...
def find_image_files(filepath, extensions=image_extensions):
    return [
        filename
        for filename in natsorted(os.listdir(filepath))
...
def find_cameras(base_dir, cameras, camera_pattern, extensions=image_extensions):
    ...
    return {
        camera: path.join(base_dir, camera_pattern.format(camera=camera))
...
def find_nonempty_dirs(filepath, extensions=image_extensions):
    return [
        local_dir
        for local_dir in os.listdir(filepath)
...
def find_images_unmatched(camera_dirs, extensions=image_extensions):
    ...
    image_filenames = [
        [path.join(camera_dir, file) for file in image_names]
...
def filenames(camera_dirs, image_names):
    return [
        [path.join(camera_dir, image) for image in image_names]
